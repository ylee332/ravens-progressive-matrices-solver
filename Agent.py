...
def find_index_of_most_similar_pixels(pixels_to_check, possible_pixels_to_match: list, thresh=0.96):
    ...
    def Solve(self, problem):
        ...
        if problem.problemType == '2x2':
            ...
            if np.array_equal(pixels_a, pixels_b):
                ...
                if get_similarity_percentages(pixels_a_horizontal_mirror, pixels_b) > 0.95:
                ...
                if get_similarity_percentages(pixels_a_horizontal_mirror, pixels_c) > 0.95:
                ...
                pixels_d = get_absolute_difference_between_pixels(pixels_d1, pixels_d2)

                for index, element in enumerate(possible_solution_pixels):
                    if np.array_equal(pixels_d, element) or np.array_equal(pixels_d1, element) or np.array_equal(
                            pixels_d2, element):
                        return index + 1
                ...
                index = find_index_of_most_similar_pixels(pixels_d, possible_solution_pixels)
                if index != -1:
                    return index
